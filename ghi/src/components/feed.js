...
function SockFeed() {
  ...
  return (
    <div className="flex flex-row place-content-between max-w-screen divide-blue divide-x-2">
      ...
      <div className="w-[100%] overflow-scroll max-h-screen">
        <div className="place-items-center align-middle grid grid-cols-3 py-20 gap-y-12">
          {data.map((sock) => {
            return (
              <div
                key={sock.id}
                className="flex flex-col items-center hover:scale-105 rounded-xl overflow-hidden shadow-2xl card col-auto w-[365px] h-[500px]"
              >
